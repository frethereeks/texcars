    ...
    <main className="relative">
      ...
      <section className="px-4 py-10 md:py-20 relative">
        ...
        <div className="container mx-auto relative z-10 space-y-16 md:space-y-32">
          <div className="bg-primary rounded-full p-4 w-max mx-auto flex items-center gap-4 text-white">
            ...
            <span className="px-6 font-bold uppercase">call us now</span>
          </div>
          <div className="gap-28 md:gap-8 flex flex-col md:flex-row-reverse">
            <form onSubmit={handleSubmit} className="grid lg:grid-cols-2 gap-x-4 w-full max-w-sm md:max-w-full mx-auto backdrop-blur-sm p-6 md:p-10">
              <input value={fullname} onChange={e => setFullname(e.target.value)} type="text" placeholder='Full Name' required className="p-4 h-max bg-backdrop/20 text-white text-base md:text-lg rounded-full border border-white hover:border-secondary placeholder:text-white hover:placeholder:text-secondary" />
              <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder='Email Address' required className="p-4 h-max bg-backdrop/20 text-white text-base md:text-lg rounded-full border border-white hover:border-secondary placeholder:text-white hover:placeholder:text-secondary" />
              <input value={location} onChange={e => setLocation(e.target.value)} type="text" placeholder='Your Current Location' required className="p-4 h-max bg-backdrop/20 text-white text-base md:text-lg rounded-full border border-white hover:border-secondary placeholder:text-white hover:placeholder:text-secondary lg:col-span-2" />
            ...
            <figure className="relative z-20 w-full md:max-w-sm space-y-8 px-6 md:px-10 pb-10 pt-20 md:pb-16 md:pt-40 text-white text-center before:w-full before:h-[200%] before:backdrop-blur-sm before:left-0 before:top-11 before:absolute before:skew-y-12 overflow-hidden">
              <div className="relative space-y-7 -translate-y-10">
                <p className="text-sm font-semibold">Emergency 24/7</p>
                <h3 className="text-3xl md:text-4xl font-bold max-w-xs mx-auto">Car Repair <span className="text-secondary">Service</span></h3>
